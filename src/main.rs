...
async fn cmd_new(contest_name: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let client = Client::builder()
        .user_agent("atcoder-rust (https://github.com/Kumasan00/Kumasan00)")
        .build()?;
        ...
        match client.get(&url).send().await {
            Ok(res) => match res.text().await {
                Ok(text) => {
                    ...
                    let bin_file_content =
                        format!("fn main() {{\n    // {name}のコードをここに書いてください\n}}");
                    fs::write(&bin_file_name, bin_file_content)?;
                }
...
fn cmd_test(contest_name: &str, problem_name: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let json_str = fs::read_to_string(&json_path)
        .map_err(|_| format!("テストケースファイルが見つかりません: {json_path}"))?;
